...
## Что показал `jbcontext analyze`

Команда была запущена с ограничением по агенту:

```bash
jbcontext analyze --agent=codex
```

На локальной истории она показала:

- 1298 задач;
- code exploration — 5% времени агента, около 9 секунд на задачу;
- прогноз для работы с JetBrains Context: на 20% меньше токенов, примерно на
  13% ниже стоимость и на 13% меньше tool calls/turns;
- ожидаемое время выполнения и доля решённых задач — без изменений.

Один и тот же запуск из трёх разных Git-репозиториев вернул одинаковые
результаты. Текущая рабочая директория не ограничивает выборку: `analyze`
агрегирует доступную локальную историю выбранного агента, а опции фильтрации по
репозиторию в протестированной версии нет.

Эти проценты нельзя считать измеренным эффектом на проверенных проектах. CLI
явно помечает их как оценку на основе `ultimate-swebench`; локального A/B-теста
с одинаковыми задачами до и после подключения JetBrains Context не проводилось.
Сразу после установки счётчики использования также ожидаемо были почти
пустыми: MCP — 0, CLI — 1, hooks — 0.
...
- Индексация привязана к Git revision, поэтому рабочие изменения нужно
  учитывать отдельно.
- `jbcontext analyze` сканирует локальную историю сессий агента. Его JSON output
  может содержать названия других проектов, количество сессий, токены,
  оценочную стоимость, размеры проектов и статистику изменений. Такой отчёт
  нельзя автоматически считать безопасным для публикации.
...
9. Запускать `jbcontext analyze --comparison` только локально, ограничивать
   нужный agent и внимательно очищать результат перед передачей третьим лицам.
   Не считать маркетинговые проценты доказанными без собственного A/B-теста.
